...
namespace BIT706AS1
{
    public class Animal
    {
        ...
        public Animal(string name, AnimalType animalType, AnimalSex? animalSex, string? breed, int? age,  MicrochipStatus? microchipStatus, String? customer)
        {
            Name = name;
        ...
        {
            if(MicrochipStatus == BIT706AS1.MicrochipStatus.Microchipped)
            {
                return Name + ", " + Sex + " " + AnimalType + ", " + MicrochipStatus + " " + $"[#{Microchip}]";
            }
            else
            {
